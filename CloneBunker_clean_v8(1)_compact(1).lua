...
local CLONE_HISTORY_FILE="PhoneIDViewer_CloneHistory.json"
...
local cloneHistory=loadJSON(CLONE_HISTORY_FILE) or {}
local favPlayerIds=loadJSON(FAV_FILE) or {}
local favItems=loadJSON(FAV_ITEMS_FILE) or {}
if type(favItems)~="table" then favItems={} end

local function addCloneHistory(playerName, userId, itemCount, mode)
    table.insert(cloneHistory, 1, {
    ...
    while #cloneHistory > 10 do table.remove(cloneHistory) end
    saveJSON(CLONE_HISTORY_FILE, cloneHistory)
end
...
local function startQuickClone(target, onDone)
    if isCloning or not target then return false end
    local items = getItems(target)
    if #items == 0 then
        showDynamicNotification("No items to clone!", T.Red)
        return false
    end
    isCloning = true
    if phone then phone.Visible = false end
    showDynamicNotification("Cloning " .. target.DisplayName .. "...", T.Gold)
    cloneItems(target, function(done, batch, total)
        if done then
            isCloning = false
            addCloneHistory(target.DisplayName, target.UserId, #items, "Quick Clone")
            showDynamicNotification("Clone complete! " .. #items .. " items", T.Green)
...
-- ==================== ORIENTASI ====================
local PHONE_SIZE_PORTRAIT = UDim2.new(0, 290, 0, 510)
...
local function applyPhoneOrientationSize()
    ...
    if landscape then
        local phoneWidth = math.floor(vp.X * 0.50)
        local phoneHeight = math.floor(vp.Y * 0.70)
        if phoneWidth > phoneHeight * 1.7 then phoneWidth = math.floor(phoneHeight * 1.7) end
        if phoneWidth < 230 then phoneWidth = 230 end
        if phoneHeight < 130 then phoneHeight = 130 end
...
local function getGridIconSize()
    if isPortrait() then
        return UDim2.new(0, 64, 0, 76)
    else
        return UDim2.new(0, 62, 0, 70)
    end
end
...
local function openPlayersApp()
    ...
    local function renderList(filter)
        ...
        for i,p in ipairs(list)do if filter==""or p.Name:lower():find(filter,1,true)or p.DisplayName:lower():find(filter,1,true)then
            ...
            if not isMe then
                local quickBtn=Instance.new("TextButton",row);quickBtn.Size=UDim2.new(0,66,0,30);quickBtn.Position=UDim2.new(1,-178,0.5,-15);quickBtn.BackgroundColor3=T.Green;quickBtn.Text="Quick Clone";quickBtn.TextColor3=T.OnAccent;quickBtn.Font=Enum.Font.GothamBold;quickBtn.TextSize=9;quickBtn.AutoButtonColor=false;corner(quickBtn,7);pressFX(quickBtn)
                quickBtn.MouseButton1Click:Connect(function()
                    if isCloning then showDynamicNotification("A clone is already running", T.Gold); return end
                    selectedPlayer=p
                    startQuickClone(p)
                end)
            end
...
-- CLONE HISTORY
local function openCloneHistoryApp()
    if #cloneHistory == 0 then
        local empty = Instance.new("TextLabel", appContent)
        empty.Size = UDim2.new(1,0,0,90)
        empty.BackgroundTransparency = 1
        empty.Text = "No clone history yet.\nClone a player to see it here."
        empty.TextColor3 = T.Text2
        empty.Font = Enum.Font.GothamBold
        empty.TextSize = 12
        empty.TextWrapped = true
        return
    end
    local clearBtn = Instance.new("TextButton", appContent)
    clearBtn.Size = UDim2.new(1,0,0,34)
    clearBtn.BackgroundColor3 = T.Card2
    clearBtn.Text = "Clear History"
    clearBtn.TextColor3 = T.Red
    clearBtn.Font = Enum.Font.GothamBold
    clearBtn.TextSize = 11
    clearBtn.AutoButtonColor = false
    corner(clearBtn,8); stroke(clearBtn,T.Border,1,0.3); pressFX(clearBtn)
    clearBtn.MouseButton1Click:Connect(function()
        cloneHistory = {}
        saveJSON(CLONE_HISTORY_FILE, cloneHistory)
        refreshCurr()
        showDynamicNotification("Clone history cleared", T.Text2)
    end)
    for i,h in ipairs(cloneHistory) do
        local row=Instance.new("Frame",appContent)
        row.Size=UDim2.new(1,0,0,64)
        row.BackgroundColor3=T.Card2
        row.LayoutOrder=i
        corner(row,10); stroke(row,T.Border,1,0.3)
        local av=Instance.new("ImageLabel",row)
        av.Size=UDim2.new(0,44,0,44); av.Position=UDim2.new(0,8,0.5,-22)
        av.BackgroundColor3=T.BG
        av.Image="https://www.roblox.com/headshot-thumbnail/image?userId="..tostring(h.userId).."&width=100&height=100&format=png"
        corner(av,100)
        local name=Instance.new("TextLabel",row)
        name.Size=UDim2.new(1,-70,0,20); name.Position=UDim2.new(0,60,0,8)
        name.BackgroundTransparency=1; name.Text=h.playerName or "Unknown"
        name.TextColor3=T.Text; name.Font=Enum.Font.GothamBold; name.TextSize=12; name.TextXAlignment=Enum.TextXAlignment.Left
        local info=Instance.new("TextLabel",row)
        info.Size=UDim2.new(1,-70,0,18); info.Position=UDim2.new(0,60,0,30)
        info.BackgroundTransparency=1; info.Text=(h.mode or "Clone").." • "..tostring(h.itemCount or 0).." items • "..(h.date or "")
        info.TextColor3=T.Text2; info.Font=Enum.Font.Gotham; info.TextSize=9; info.TextXAlignment=Enum.TextXAlignment.Left
    end
end
...
local function openCloneApp()
    ...
    cloneBtn.MouseButton1Click:Connect(function()
        ...
        local bar=Instance.new("Frame",appContent);bar.Size=UDim2.new(1,0,0,10);bar.BackgroundColor3=T.Card2;corner(bar,5)
        local fill=Instance.new("Frame",bar);fill.Size=UDim2.new(0,0,1,0);fill.BackgroundColor3=T.Green;corner(fill,5)
        local progressLbl=Instance.new("TextLabel",appContent);progressLbl.Size=UDim2.new(1,0,0,24);progressLbl.BackgroundTransparency=1;progressLbl.Text="Starting clone...";progressLbl.TextColor3=T.Text2;progressLbl.Font=Enum.Font.GothamBold;progressLbl.TextSize=10;progressLbl.TextXAlignment=Enum.TextXAlignment.Center
        cloneItems(selectedPlayer,function(done,batch,total)if done then isCloning=false;cloneBtn.Text="Clone Done";tween(cloneBtn,{BackgroundColor3=T.Green},0.3);tween(fill,{Size=UDim2.new(1,0,1,0)},0.2);progressLbl.Text="100% • "..#items.." items cloned";addCloneHistory(selectedPlayer.DisplayName, selectedPlayer.UserId, #items, "Clone");showDynamicNotification("Clone complete!",T.Green)else local r=batch/total;tween(fill,{Size=UDim2.new(r,0,1,0)},0.3);cloneBtn.Text=("Cloning %d/%d"):format(batch,total);progressLbl.Text=("%d%% • Batch %d/%d"):format(math.floor(r*100),batch,total)end end)
    end)
...
buildAppIcon("History",8,appGrid,function() openApp("Clone History",openCloneHistoryApp) end)
